...
import io.github.numq.haskcore.api.clipboard.ClipboardApi
...
class CopySessionText(private val clipboardApi: ClipboardApi) : UseCase<CopySessionText.Input, Unit> {
    data class Input(val session: OutputSession)

    override suspend fun Raise<Throwable>.execute(input: Input) = clipboardApi.copyToClipboard(
