...
class CopySessionText(private val clipboardService: ClipboardService) : UseCase.Command<CopySessionText.Input> {
    data class Input(val session: OutputSession)

    override suspend fun Raise<Throwable>.command(input: Input) = clipboardService.copyToClipboard(
