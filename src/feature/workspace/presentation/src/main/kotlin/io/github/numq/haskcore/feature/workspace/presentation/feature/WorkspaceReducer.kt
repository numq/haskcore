...
internal class WorkspaceReducer(
    ...
    private val toggleFullscreen: ToggleFullscreen,
) : Reducer<WorkspaceState, WorkspaceCommand, WorkspaceEvent> {
    override fun reduce(state: WorkspaceState, command: WorkspaceCommand): Transition<WorkspaceState, WorkspaceEvent> =
        when (command) {
                ...
                action(
                    key = command.key, fallback = WorkspaceCommand::HandleFailure, block = {
                        closeWorkspace(input = Unit).fold(
                            ifLeft = WorkspaceCommand::HandleFailure, ifRight = {
                                WorkspaceCommand.CloseDocumentSuccess
                            })
                    })
            )
                ...
                action(
                    key = command.key, fallback = WorkspaceCommand::HandleFailure, block = {
                        openDocument(input = OpenDocument.Input(path = command.document.path)).fold(
                            ...
                            ifRight = { WorkspaceCommand.OpenDocumentSuccess })
                    })
            )
                ...
                action(
                    key = command.key, fallback = WorkspaceCommand::HandleFailure, block = {
                        closeDocument(input = CloseDocument.Input(path = command.document.path)).fold(
                            ifLeft = WorkspaceCommand::HandleFailure,
                            ifRight = { WorkspaceCommand.CloseDocumentSuccess })
                    })
            )
                ...
                action(
                    key = command.key, fallback = WorkspaceCommand::HandleFailure, block = {
                        selectShelfTool(input = SelectShelfTool.Input(tool = command.tool)).fold(
                            ifLeft = WorkspaceCommand::HandleFailure,
                            ifRight = { WorkspaceCommand.SelectShelfToolSuccess })
                    })
            )
                ...
                action(
                    key = command.key, fallback = WorkspaceCommand::HandleFailure, block = {
                        val input = with(command) {
                            SaveDimensions.Input(x = x, y = y, width = width, height = height)
                        }

                        saveDimensions(input = input).fold(
                            ifLeft = WorkspaceCommand::HandleFailure,
                            ifRight = { WorkspaceCommand.SaveDimensionsSuccess })
                    })
            )

            is WorkspaceCommand.SaveDimensionsSuccess -> transition(state)

            is WorkspaceCommand.ToggleFullscreen -> transition(state).effect(
                action(
                    key = command.key, fallback = WorkspaceCommand::HandleFailure, block = {
                        toggleFullscreen(input = Unit).fold(
                            ifLeft = WorkspaceCommand::HandleFailure,
                            ifRight = { WorkspaceCommand.ToggleFullscreenSuccess })
                    })
            )

            is WorkspaceCommand.ToggleFullscreenSuccess -> transition(state)

            is WorkspaceCommand.Minimize -> transition(state)

            is WorkspaceCommand.ToggleMaximize -> transition(state)

            is WorkspaceCommand.Close -> with(command) {
                when (state) {
                    is WorkspaceState.Loading -> transition(state)

                    is WorkspaceState.Ready -> when {
                        state.workspace.isFullscreen -> reduce(
                            state = state, command = WorkspaceCommand.SaveDimensions(
                                x = state.workspace.x,
                                y = state.workspace.y,
