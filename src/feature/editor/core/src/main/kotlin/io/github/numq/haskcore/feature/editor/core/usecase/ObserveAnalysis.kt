...
class ObserveAnalysis(
    ...
    private suspend fun observeAnalysis(path: String) {
        toolchainService.toolchain.filterIsInstance<Toolchain.Detected>().mapNotNull { toolchain ->
            ...
            lspService.start(hlsPath = hlsPath).getOrElse(::println)

            lspService.connection.flatMapLatest { connection ->
                when (connection) {
                    is LspConnection.Connected -> {
                        val currentSnapshot = textService.snapshot.value

                        if (currentSnapshot != null) {
                            lspService.openDocument(path = path, text = currentSnapshot.text)
            ...
            is Language.Haskell -> channelFlow {
                ...
                combine(
                    ...
                    transform = { snapshot, completions, diagnostics ->
                        val issues = diagnostics.filter { diagnostic ->
