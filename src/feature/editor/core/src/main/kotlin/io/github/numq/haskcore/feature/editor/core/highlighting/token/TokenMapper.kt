...
internal object TokenMapper {
    ...
    fun parseSemantic(type: String): SyntaxTokenType = when (type) {
        "parameter" -> SyntaxTokenType.VARIABLE_PARAMETER

        "property", "enumMember" -> SyntaxTokenType.VARIABLE_FIELD

        "function", "method" -> SyntaxTokenType.FUNCTION

        "class", "type", "struct" -> SyntaxTokenType.TYPE

        "variable" -> SyntaxTokenType.VARIABLE

        "keyword" -> SyntaxTokenType.KEYWORD

        "operator" -> SyntaxTokenType.OPERATOR

        "comment" -> SyntaxTokenType.COMMENT

        "number" -> SyntaxTokenType.NUMBER

        "string" -> SyntaxTokenType.STRING

        else -> SyntaxTokenType.VARIABLE
    }
}