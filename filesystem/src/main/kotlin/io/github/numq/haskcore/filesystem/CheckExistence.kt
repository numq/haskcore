...
class CheckExistence(private val fileSystem: FileSystemService) : UseCase<CheckExistence.Input, Boolean> {
    data class Input(val path: String)

    override suspend fun execute(input: Input) = fileSystem.exists(path = input.path)
}